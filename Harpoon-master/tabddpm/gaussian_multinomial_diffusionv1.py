...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.num_classes_expanded = torch.from_numpy(
            np.concatenate(
                [np.repeat(int(self.num_classes[i]), int(self.num_classes[i])) for i in range(len(self.num_classes))])
        ).float().to(device)
    ...
    def mixed_loss(self, x, out_dict, encode_again=True):
        b = x.shape[0]
        device = x.device
        ...
        if x_cat.shape[1] > 0:
            log_x_cat = index_to_log_onehot(x_cat.long(), self.num_classes, encode_again)
