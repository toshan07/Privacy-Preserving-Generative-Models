...
def download_from_uci(name):
    ...
    if not os.path.exists(save_dir):
        ...
        if name == 'california':
            unzip_file(f'{DATA_DIR}/california-housing-prices.zip', save_dir)
            print(f'Data has been saved to {save_dir}.')
        else:
...
def train_test_split(dataname, ratio=0.7):
    ...
    path = f'{DATA_DIR}/{dataname}/data.csv' if dataname != 'california' else f'{DATA_DIR}/{dataname}/housing.csv'
