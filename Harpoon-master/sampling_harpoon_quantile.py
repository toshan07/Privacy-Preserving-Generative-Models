...
if __name__ == '__main__':
    ...
    qt.fit(train_X[:, :num_numeric])
    # X = (train_X - mean_X) / std_X
    X = np.concatenate((qt.transform(train_X[:, :num_numeric]), train_X[:, num_numeric:]), axis=1)
    X = torch.tensor(X)
    X_test_eval = (test_X - mean_X) / std_X
    X_test = np.concatenate((qt.transform(test_X[:, :num_numeric]), test_X[:, num_numeric:]), axis=1)
    ...
    model_path = os.path.join(models_dir, "diffputer_selfmade_quantile.pt")
    ...
    with (torch.no_grad()):
        for trial in tqdm(range(num_trials), desc='Out-of-sample imputation'):
            ...
            X_true_eval = X_test_eval
            X_pred_eval = np.concatenate((qt.inverse_transform(X_pred[:, :num_numeric]), X_pred[:, num_numeric:]), axis=1)
            X_pred_eval = (X_pred_eval - mean_X)/std_X  # convert all evaluations using the same standard scaler for proper comparison
            X_true_dec = prepper.decodeNp(scheme='OHE', arr=X_true_eval)
            X_pred_dec = prepper.decodeNp(scheme='OHE', arr=X_pred_eval)
            if args.ignore_hard_fix:
    ...
    method_str = f'harpoon_ohe_{args.loss}_linear_quantile' if args.schedule else f'harpoon_ohe_{args.loss}_quantile'
    if args.ignore_hard_fix:
        method_str += 'softfix'
