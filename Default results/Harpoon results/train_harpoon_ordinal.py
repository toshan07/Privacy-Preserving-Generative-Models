...
parser.add_argument('--dataname', type=str, default='default', help='Name of dataset.')
...
if __name__ == '__main__':
    ...
    target_categories = pd.unique(prepper.df.iloc[:, target_idx])
    target_codes = pd.Categorical(
        prepper.df_train.iloc[:, target_idx], categories=target_categories
    ).codes.astype(np.float32)
    if np.any(target_codes < 0):
        raise ValueError('Found unknown target labels in training split.')

    train_X = np.concatenate([train_X_base, target_codes.reshape(-1, 1)], axis=1)
    ...
    torch.save(model.state_dict(), f'{models_dir}/harpoon_ordinal_with_target.pt')

    plot_path = f'{models_dir}/harpoon_ordinal_with_target_loss_curve.png'
    plt.figure(figsize=(8, 5))
    plt.plot(range(1, len(epoch_losses) + 1), epoch_losses)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Training Loss vs Epoch ({dataname})')
    plt.tight_layout()
